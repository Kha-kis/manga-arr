...
async def _grab_existing_inner(series_id: int, title: str, pattern: str) -> int:
    ...
    log_event(
        "search",
        f"Search '{title}': {len(all_items)} candidates, {grabbed} grabbed",
        series_id,
    )
    return grabbed
...
async def search_complete_pack(
    series_id: int, title: str, total_volumes: int | None
) -> int:
    ...
    useful_aliases = [
        a for a in aliases if _useful_search_term(a) and normalize(a) != norm_title
    ]
    useful_aliases.sort(
