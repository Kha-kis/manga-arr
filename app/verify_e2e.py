...
def check_grabbed_volumes(db) -> list[Finding]:
    ...
    findings = [Finding("grabbed_volumes_total", "info",
                        f"grabbed volumes: {total}", total, missing)]
    for col, n in missing.items():
        if n:
            findings.append(Finding(
                f"grabbed_volume_missing_{col}", "warning",
                f"{n} grabbed volumes missing '{col}'", n,
            ))
    return findings
...
def check_downloaded_volumes(db) -> list[Finding]:
    ...
    findings = [Finding("downloaded_volumes_total", "info",
                        f"downloaded volumes: {total}", total, missing)]
    for col, n in missing.items():
        if n:
            findings.append(Finding(
                f"downloaded_volume_missing_{col}", "warning",
                f"{n} downloaded volumes missing '{col}'", n,
            ))
    return findings
...
def check_grabbed_chapters(db) -> list[Finding]:
    ...
    findings = [Finding("grabbed_chapters_total", "info",
                        f"grabbed chapters: {total}", total, missing)]
    for col, n in missing.items():
        if n:
            findings.append(Finding(
                f"grabbed_chapter_missing_{col}", "warning",
                f"{n} grabbed chapters missing '{col}'", n,
            ))
    return findings
...
def check_downloaded_chapters(db) -> list[Finding]:
    ...
    total = _i(r['total'])
    ghost = _i(r['ghost'])
    findings = [Finding("downloaded_chapters_total", "info",
                        f"downloaded chapters: {total}", total)]
    if ghost:
        # Ghost = marked downloaded but no file path + no quality. Operator
        # action required to either re-grab or mark blocklisted.
        findings.append(Finding(
            "ghost_downloaded_chapters", "warning",
            f"{ghost} ghost downloaded chapters (no file, no quality)", ghost,
        ))
    for col, key in (("no_quality", "quality"),
                     ("no_import_path", "import_path"),
                     ("no_imported_at", "imported_at")):
        n = _i(r[col])
        if n and n != ghost:  # ghost row already covers the both-missing case
            findings.append(Finding(
                f"downloaded_chapter_missing_{key}", "warning",
                f"{n} downloaded chapters missing '{key}'", n,
            ))
    return findings
...
def check_import_queue(db) -> list[Finding]:
    ...
    findings.append(Finding(
        "import_queue_states", "info",
        f"import queue states: {by_status or '(empty)'}",
        sum(by_status.values()), {"by_status": by_status},
    ))
    # Failed and stuck "importing" rows are operator-actionable.
    for status, sev in (("failed", "warning"), ("importing", "warning"),
                        ("partial", "warning")):
        n = by_status.get(status, 0)
        if n:
            findings.append(Finding(
                f"import_queue_{status}", sev,
                f"{n} import-queue rows in '{status}' state", n,
            ))
    return findings
...
def check_stuck_grabbed(db, threshold_days: int = 2) -> list[Finding]:
    ...
    findings = [Finding(
        "stuck_grabbed_total", "info",
        f"stuck grabbed volumes (>{threshold_days}d): {n}", n,
        {"threshold_days": threshold_days},
    )]
    if n:
        findings.append(Finding(
            "stuck_grabbed_warning", "warning",
            f"{n} volumes stuck in 'grabbed' state for >{threshold_days} days; "
            "either downloader lost them or import never picked them up",
            n, {"threshold_days": threshold_days},
        ))
    return findings
...
def check_settings(db) -> list[Finding]:
    ...
    for key in ("blocklist_ttl_days", "api_key"):
        ...
        if key == "api_key":
            if v and v.strip():
                findings.append(Finding("api_key_present", "info",
                                        "api_key: set", 1))
            else:
                # Blank api_key is a *critical* security regression.
                findings.append(Finding(
                    "api_key_blank", "critical",
                    "api_key is blank or missing — /api routes will reject all requests",
                    0,
                ))
        else:
            findings.append(Finding(f"setting_{key}", "info",
                                    f"{key}: {v if v is not None else '(missing)'}", 0))
    return findings
...
def check_orphans(db) -> list[Finding]:
    ...
    findings.append(Finding(
        "orphan_chapters", "critical" if n_ch else "info",
        f"orphan chapters (volume_id → missing volume): {n_ch}", n_ch,
    ))
    n_vol = _i(db.execute(
    ...
    findings.append(Finding(
        "orphan_volumes", "critical" if n_vol else "info",
        f"orphan volumes (series_id → missing series): {n_vol}", n_vol,
    ))
    return findings
...
def diagnose(db_path: str = DEFAULT_DB_PATH) -> list[Finding]:
    ...
    if not os.path.isfile(db_path):
        return [Finding("db_missing", "critical",
                        f"database not found at {db_path}", 0)]
...
def _print_legacy(findings: list[Finding]) -> None:
    ...
    print(f"    no import_path: {_pick('downloaded_chapter_missing_import_path') + ghost}")
    ...
    print("\n[8] Settings:")
    for f in findings:
        if f.code.startswith("setting_") or f.code in ("api_key_present", "api_key_blank"):
