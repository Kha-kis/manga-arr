...
async def grab_item(item: dict, series_id: int, respect_monitoring: bool = True) -> bool:
    ...
    with get_db() as db:
        ...
        if _release_guid is not None and db.execute(
            ...
            grab_dedup._log_grab_rejection(series_id, title, f'duplicate release_guid: {_release_guid}')
            return False
        ...
        if existing_vol:
            ...
            if _revision['is_repack']:
                if _prop_cfg == 'do_not_upgrade':
                    ...
                    grab_dedup._log_grab_rejection(series_id, title,
            ...
            cutoff = (_s_cutoff['quality_cutoff'] if _cutoff_db else None) or get_cfg('quality_cutoff', '')
