...
def _plan_import(
    ...
    try:
        ...
        log_event(
            "error", f"Import: cannot create {dst_dir}: {e}", queue["series_id"], db=db
        )
...
def _process_import_file(
    ...
    if fp.file_type == "chapter" and fp.proposed_chap is not None:
        _process_chapter_import(
            db, fp, dst, plan, queue, series_id, imported_vols, chapter_vols_touched
        )
    else:
...
def _mark_downloaded(db, series_id, volume_num, torrent_url) -> bool:
    ...
    if volume_num is not None:
        ...
        if cur.rowcount > 0:
            ...
            log_event(
                "download_complete", f"{label} pack download complete", series_id, db=db
            )
            s = db.execute(
                "SELECT title, cover_url FROM series WHERE id=?", (series_id,)
            ).fetchone()
