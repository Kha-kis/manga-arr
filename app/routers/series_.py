...
async def _grab_volume_task(series_id: int, s, v, query: str):
    ...
    general = (
        await _m._search_all(s["title"], purpose="interactive", series_id=series_id)
        if query != s["title"]
        else []
    )
...
async def index(
    ...
    with get_db() as db:
        ...
        order = build_order_by(
            sort, allowed=_LIBRARY_SORT_ALLOWED, default_key=_LIBRARY_SORT_DEFAULT
        )
        ...
        if filter_missing == "1":
            ...
            series = [
                x
                for x in series
                if x["stats"]["wanted"] == 0 and x["stats"]["total"] > 0
            ]

        if sort == "progress":
            series.sort(
                key=lambda x: -(x["stats"]["have"] / max(x["stats"]["total"], 1))
            )
        elif sort == "missing":
        ...
        if pt == "complete":
            ...
            if rs is not None and re_ is not None:
                ...
                if rng:
                    rs, re_ = rng
                    vol_label = (
                        f"Vol {vol_num_to_display(rs)}–{vol_num_to_display(re_)}"
                    )
                    covers = set(range(int(rs), int(re_) + 1))
                elif total_vols:
...
async def api_series_reconcile_apply(request: Request, series_id: int):
    ...
    with get_db() as _db:
        exists = _db.execute("SELECT 1 FROM series WHERE id=?", (series_id,)).fetchone()
    if not exists:
        return JSONResponse({"error": "series not found"}, status_code=404)
...
async def search(request: Request, q: str = ""):
    ...
    with get_db() as db:
        ...
        for r in db.execute(
            ...
            existing_anilist.setdefault(r["anilist_id"], []).append(
                r["edition_type"] or "standard"
            )
        existing_mu = {
            r["mu_id"]
            for r in db.execute(
...
async def add_series(
    ...
    with get_db() as db:
        ...
        if (
            total_volumes
            and total_volumes > 0
            and edition_type not in _m._NON_STANDARD_STUB_EDITIONS
        ):
    ...
    return RedirectResponse(
        with_flash(
            f"/series/{series_id}", "Search queued for all wanted volumes", "success"
        ),
        status_code=303,
    )
        ...
        return _Resp(
            headers={
                "HX-Trigger": json.dumps(
                    {
                        "showToast": {
                            "msg": f"{title} moved to recycle bin"
                            if title
                            else "Series moved to recycle bin",
                            "type": "success",
                            "actionLabel": "Undo",
        ...
        return _Resp(
            headers={
                "HX-Trigger": json.dumps(
                    {
                        "showToast": {
                            "msg": "Series permanently deleted",
                            "type": "success",
                        }
                    }
                ),
        ...
        return _Resp(
            headers={
                "HX-Trigger": json.dumps(
                    {
                        "showToast": {
                            "msg": f"Restored {restored} series"
                            if restored
...
async def manual_grab(request: Request, series_id: int):
    ...
    if s:
        _m.create_background_task(
            ...
            name=f"series:{series_id}:manual_grab",
        )
    if request.headers.get("HX-Request") == "true":
        import json
        from fastapi.responses import Response as _Resp
        ...
        return _Resp(
            headers={
                "HX-Trigger": json.dumps(
                    {
                        "showToast": {
                            "msg": "Search queued for all wanted volumes",
                            "type": "success",
                        }
                    }
                )
...
async def grab_volume(request: Request, series_id: int, volume_id: int):
    ...
    if s and v:
        ...
        if ddl_mode == "only" and ddl_available:
            ...
            if not ddl_ok:
                vol_q = (
                    f"{s['title']} v{vol_num_to_display(v['volume_num'])}"
                    if v["volume_num"]
                    else s["title"]
                )
                import main as _m
                ...
                _m.create_background_task(
                    _grab_volume_task(series_id, s, v, vol_q),
                    name=f"series:{series_id}:grab_volume:{volume_id}",
                )
...
async def toggle_monitored(request: Request, series_id: int):
    with get_db() as db:
        cur = db.execute(
            "SELECT monitored FROM series WHERE id=?", (series_id,)
        ).fetchone()
        if cur:
        ...
        return _Resp(
            headers={
                "HX-Trigger": json.dumps(
                    {
                        "showToast": {
                            "msg": "Metadata refresh started in background",
                            "type": "success",
                        }
                    }
                )
...
async def chapter_map_editor(request: Request, series_id: int):
    with get_db() as db:
        ...
        merged = {
            **ch_map,
            **{k: str(v) if v is not None else None for k, v in overrides.items()},
        }
    ...
    return templates.TemplateResponse(
        request,
        "chapter_map_editor.html",
        {
            "request": request,
            "s": s,
            "ch_map": merged,
            "ch_map_items": ch_map_items,
            "overrides": overrides,
...
async def save_chapter_map(request: Request, series_id: int):
    ...
    with get_db() as db:
        db.execute(
            "DELETE FROM series_chapter_overrides WHERE series_id=?", (series_id,)
        )
...
async def edit_series(request: Request, series_id: int):
    ...
    plain_fields = {
        ...
        "omnibus_preference": (
            ...
            lambda v: (
                v
                if (v := str(v or "").strip()) in _VALID_OMNIBUS_PREFS
        ...
        "quality_cutoff": (
            "quality_cutoff",
            lambda v: (
                v if (v := str(v or "").strip()) in _VALID_QUALITY_CUTOFFS else ""
            ),
        ),
        "update_strategy": (
            "update_strategy",
            lambda v: (
                v
                if (v := str(v or "").strip()) in _VALID_UPDATE_STRATEGIES
                else "always"
            ),
        ),
    ...
    with get_db() as db:
        ...
        if "total_volumes" in submitted:
            ...
            if tv > 0:
                ...
                _manual_new = tv
                updates.append("total_volumes=?")
                params.append(_manual_new)
                updates.append("vol_count_source=?")
        ...
        if _manual_new is not None:
            if _manual_old and _manual_new < _manual_old:
                ...
                _m.log_event(
                    "metadata",
                    f"[Manual] removed wanted stubs > vol {_manual_new}",
                    series_id,
                )
...
async def mark_volume_downloaded(request: Request, series_id: int, volume_id: int):
    ...
    with get_db() as db:
        ...
        if v and s:
            vol_label = (
                f"Vol {vol_num_to_display(v['volume_num'])}" if v["volume_num"] else "—"
            )
            _m.add_history(
                db, "volume_marked_downloaded", series_id, s["title"], vol_label
            )
    if request.headers.get("HX-Request") == "true":
        ctx = await _get_volume_row_ctx(series_id, volume_id)
        return templates.TemplateResponse(request, "partials/volume_row.html", ctx)
...
async def toggle_volume_monitor(request: Request, series_id: int, volume_id: int):
    with get_db() as db:
        v = db.execute(
            "SELECT monitored FROM volumes WHERE id=? AND series_id=?",
            (volume_id, series_id),
        ).fetchone()
        if v:
            db.execute(
...
async def set_series_root_folder(
    request: Request, series_id: int, root_folder_id: int = Form(0)
):
    with get_db() as db:
        db.execute(
        ...
        return _Resp(
            headers={
                "HX-Trigger": json.dumps(
                    {
                        "showToast": {
                            "msg": f"Monitor mode: {labels.get(mode, mode)}",
                            "type": "success",
                        }
                    }
                )
...
async def search_volume_releases(series_id: int, volume_id: int):
    ...
    with get_db() as db:
        ...
        alias_rows = (
            db.execute(
                "SELECT alias FROM series_aliases WHERE series_id=?", (s["id"],)
            ).fetchall()
            if s
            else []
        )
    ...
    if s["search_pattern"] not in (query, s["title"]):
        sp_q = (
            f"{s['search_pattern']} v{vol_num_to_search(vol_num)}"
            if vol_num
            else s["search_pattern"]
        )
        queries.append(sp_q)
    for a in alias_rows[:3]:
        alias_q = (
            f"{a['alias']} v{vol_num_to_search(vol_num)}" if vol_num else a["alias"]
        )
    ...
    all_results = await asyncio.gather(
        *[
            _m._search_all(q, purpose="interactive", series_id=series_id)
            for q in queries
        ]
    )
    ...
    return JSONResponse(
        {"results": results, "query": query, "suwayomi_available": suwayomi_available}
    )
...
async def grab_volume_ddl(series_id: int, volume_id: int):
    ...
    if not v or not v["volume_num"]:
        return JSONResponse(
            {"ok": False, "message": "Volume not found"}, status_code=404
        )
    from routers import suwayomi_ as _swy
...
async def mark_all_grabbed_downloaded(request: Request, series_id: int):
    ...
    _m.log_event(
        "download_complete",
        "Manually marked all grabbed volumes as downloaded",
        series_id,
    )
    if request.headers.get("HX-Request") == "true":
        from fastapi.responses import Response as _Resp
...
async def add_series_alias(request: Request, series_id: int, alias: str = Form("")):
    ...
    if request.headers.get("HX-Request") == "true":
        with get_db() as db:
            aliases = db.execute(
                "SELECT * FROM series_aliases WHERE series_id=? ORDER BY alias",
                (series_id,),
            ).fetchall()
        return templates.TemplateResponse(
            request,
...
async def delete_series_alias(request: Request, series_id: int, alias_id: int):
    with get_db() as db:
        db.execute(
            "DELETE FROM series_aliases WHERE id=? AND series_id=?",
            (alias_id, series_id),
        )
    if request.headers.get("HX-Request") == "true":
        with get_db() as db:
...
async def refresh_series_mangadex(request: Request, series_id: int):
    ...
    if ok:
        _m.backfill_pack_ranges()
        _m.log_event(
            "refresh", "MangaDex chapter map refreshed and backfill applied", series_id
        )
    if request.headers.get("HX-Request") == "true":
        import json
        from fastapi.responses import Response as _Resp
...
async def refresh_edition_metadata(series_id: int):
    ...
    if count is None:
        return JSONResponse(
            {
                "ok": False,
                "message": "Could not determine volume count from Google Books",
            }
        )
...
async def toggle_chapter_monitor(request: Request, sid: int, cid: int):
    with get_db() as db:
        ch = db.execute(
            "SELECT monitored, volume_id FROM chapters WHERE id=? AND series_id=?",
            (cid, sid),
        ).fetchone()
        if ch:
            db.execute(
                "UPDATE chapters SET monitored=? WHERE id=?",
...
async def mark_chapter_downloaded(request: Request, sid: int, cid: int):
    ...
    with get_db() as db:
        ch = db.execute(
            "SELECT chapter_num, volume_id FROM chapters WHERE id=?", (cid,)
        ).fetchone()
        ...
        if ch and ch["volume_id"]:
            ...
            db.execute(
                ...
                (
                    now_iso,
                    _sib.get("import_path"),
                    _sib.get("quality"),
                    _sib.get("torrent_name"),
                    _sib.get("indexer"),
                    _sib.get("protocol"),
                    _sib.get("client"),
                    _sib.get("release_group"),
        ...
        if ch and s:
            ch_label = (
                f"Ch {ch['chapter_num']}" if ch["chapter_num"] is not None else "—"
            )
...
async def grab_chapter_route(request: Request, sid: int, cid: int):
    ...
    if not s or not ch:
        return RedirectResponse(
            with_flash(f"/series/{sid}", "No wanted chapters found", "info"),
            status_code=303,
        )
    import main as _m

    _m.create_background_task(
        _grab_chapter_task(sid, dict(s), dict(ch)),
        name=f"series:{sid}:grab_chapter:{cid}",
    )
    if request.headers.get("HX-Request") == "true":
        if ch["volume_id"]:
            ctx = await _get_volume_row_ctx(sid, ch["volume_id"])
        ...
        return _Resp(headers={"HX-Refresh": "true"})
    return RedirectResponse(
        with_flash(f"/series/{sid}", "Grab queued for 1 chapter", "success"),
        status_code=303,
    )
...
async def uncollected_toggle_monitor(request: Request, sid: int):
    with get_db() as db:
        current = db.execute(
            "SELECT monitored FROM chapters WHERE series_id=? AND volume_id IS NULL LIMIT 1",
            (sid,),
        ).fetchone()
    ...
    if request.headers.get("HX-Request") == "true":
        ...
        return _Resp(headers={"HX-Refresh": "true"})
    msg = (
        "Uncollected chapters monitored"
        if new_val
        else "Uncollected chapters unmonitored"
    )
    return RedirectResponse(
        with_flash(f"/series/{sid}", msg, "success"), status_code=303
    )
...
async def uncollected_grab_all(request: Request, sid: int):
    ...
    for ch in chs:
        ...
        _m.create_background_task(
            _grab_chapter_task(sid, dict(s), dict(ch)),
            name=f"series:{sid}:grab_chapter:{ch['id']}",
        )
    if request.headers.get("HX-Request") == "true":
        import json
        from fastapi.responses import Response as _Resp
        ...
        return _Resp(
            headers={
                "HX-Trigger": json.dumps(
                    {
                        "showToast": {
                            "msg": f"Grab queued for {len(chs)} chapters",
                            "type": "success",
                        }
                    }
                )
...
async def trigger_volume_upgrade(
    request: Request, sid: int, vol_id: int, redirect_to: str = Form("/calendar")
):
    with get_db() as db:
        s = db.execute("SELECT * FROM series WHERE id=?", (sid,)).fetchone()
        v = db.execute(
            "SELECT * FROM volumes WHERE id=? AND series_id=?", (vol_id, sid)
        ).fetchone()
    ...
    _m.create_background_task(
        _grab_volume_task(sid, s, v, query),
        name=f"series:{sid}:upgrade_volume:{vol_id}",
    )
    if request.headers.get("HX-Request") == "true":
        ctx = await _get_volume_row_ctx(sid, vol_id)
        return templates.TemplateResponse(request, "partials/volume_row.html", ctx)
...
async def grab_all_wanted_for_series(request: Request, sid: int):
    ...
    for v in wanted:
        ...
        _m.create_background_task(
            _grab_volume_task(sid, dict(s), dict(v), query),
            name=f"series:{sid}:grab_volume:{v['id']}",
        )
    if request.headers.get("HX-Request") == "true":
        import json
        from fastapi.responses import Response as _Resp
        ...
        return _Resp(
            headers={
                "HX-Trigger": json.dumps(
                    {
                        "showToast": {
                            "msg": f"Search queued for {len(wanted)} volumes",
                            "type": "success",
                        }
                    }
                )
