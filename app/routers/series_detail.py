...
from app.shared import get_cfg, get_db, get_root_folders, quality_rank, vol_num_to_display
...
async def mark_all_grabbed_downloaded(request: Request):
    ...
    with get_db() as db:
        ...
        marked = cur.rowcount
    _m.log_event('download_complete', f"Manually marked all grabbed volumes as downloaded ({marked})", 0)
    if request.headers.get("HX-Request") == "true":
