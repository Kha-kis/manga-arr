...
NOTIFICATION_FIELD_SCHEMAS: dict[str, list[dict]] = {
    ...
    "telegram": [
        ...
        },
        {
            ...
            "help": "Target chat or channel ID.",
        },
    ],
    "slack": [
        {
            ...
            "help": "Slack incoming webhook endpoint.",
        },
    ],
    "ntfy": [
        {
            ...
            "help": "Defaults to https://ntfy.sh.",
        ...
        },
        {
            ...
            "secret": False,
            "placeholder": "mangarr",
            "help": "Topic to publish notifications to.",
        },
        {
            "name": "token",
            "label": "Access Token",
    ...
    "gotify": [
        {
            ...
            "help": "Base URL of the Gotify server.",
        },
        {
            "name": "app_token",
            "label": "App Token",
            ...
            "help": "Token from the Gotify application.",
        },
    ],
    "pushover": [
        {
    ...
    "email": [
        ...
        },
        {
            ...
            "required": False,
            "secret": False,
            "placeholder": "mangarr@localhost",
            "help": "Envelope sender used for notifications.",
            "default": "mangarr@localhost",
        },
        {
            "name": "to",
            "label": "To Address",
...
def _notification_settings_from_form(
    form,
    ctype: str,
    *,
...
async def create_notification_connection(
    ...
    events = {}
    for flag, _ in EVENT_FLAGS:
        raw = form.get(flag, 0)
        assert isinstance(raw, (str, int))
        events[flag] = int(raw)
    ...
    with get_db() as db:
        db.execute(
            ...
            (
                ...
                events.get("on_grab", 1),
                events.get("on_download", 1),
                events.get("on_upgrade", 1),
                events.get("on_series_add", 1),
                events.get("on_health_issue", 1),
                events.get("on_health_restored", 0),
            ),
        )
...
async def test_notification_connection(conn_id: int):
    ...
    ok, msg = await send_connection(
        dict(conn), "Test notification from Mangarr", event="test"
    )
...
async def test_notification_connection_form(
    ...
    ok, msg = await send_connection(
        {
            "name": name.strip() or "Unsaved notification",
            "type": type,
            "settings": json.dumps(settings_dict or {}),
        },
...
async def send_connection(
    conn: dict, message: str, event: str = "", embed: dict | None = None
) -> tuple[bool, str]:
...
async def _send_webhook(
    s: dict, message: str, event: str, embed: dict | None
) -> tuple[bool, str]:
    url = s.get("url", "")
