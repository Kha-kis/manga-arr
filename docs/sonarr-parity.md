...
coverage includes host, media-management, UI, naming, quality,
...
controls, URL-base storage, selected UI/naming config reads, selected
environment defaults, outbound proxy deployment guidance, and deployment
documentation. Sonarr also exposes broader settings for analytics, updates,
detailed syslog, UI style preferences, and server options.
