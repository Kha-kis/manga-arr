...
@_skip_unless("PROWLARR_LIVE", "Prowlarr live test (set PROWLARR_LIVE=1 + PROWLARR_URL + PROWLARR_API_KEY)")
def test_prowlarr_system_status():
    ...
    r = httpx.get(f"{url}/api/v1/system/status",
...
@_skip_unless("QBITTORRENT_LIVE", "qBittorrent live test (set QBITTORRENT_LIVE=1 + QBIT_HOST + QBIT_USER + QBIT_PASS)")
def test_qbittorrent_auth_and_version():
    ...
    with httpx.Client(timeout=10) as c:
        r = c.post(f"{host}/api/v2/auth/login",
...
@_skip_unless("SABNZBD_LIVE", "SABnzbd live test (set SABNZBD_LIVE=1 + SAB_HOST + SAB_API_KEY)")
...
@_skip_unless("SUWAYOMI_LIVE", "Suwayomi live test (set SUWAYOMI_LIVE=1 + SUWAYOMI_URL)")
