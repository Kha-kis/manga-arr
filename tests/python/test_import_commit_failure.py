...
def test_commit_all_failure_marks_import_failed(test_env, monkeypatch):
    ...
    with sqlite3.connect(test_env["db_path"]) as c:
        ...
        vol_downloaded = c.execute(
            "SELECT COUNT(*) FROM volumes WHERE series_id=? AND status='downloaded' AND import_path IS NOT NULL",
            (sid,),
        ).fetchone()[0]
        assert vol_downloaded == 0, (
            "No volumes should be marked as downloaded after commit failure"
        )

        chap_downloaded = c.execute(
            "SELECT COUNT(*) FROM chapters WHERE series_id=? AND status='downloaded' AND import_path IS NOT NULL",
            (sid,),
        ).fetchone()[0]
...
def test_commit_failure_prevents_post_success_side_effects(test_env, monkeypatch):
    ...
    assert side_effects_called["success_history"] is False, (
        "Success history should not be added on commit failure"
    )
