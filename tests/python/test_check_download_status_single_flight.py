...
def fresh_db():
    ...
    import_discovery._CHECK_DOWNLOAD_STATUS_LOCK = asyncio.Lock()
...
def test_lock_exists_as_module_attribute():
    ...
    assert isinstance(import_discovery._CHECK_DOWNLOAD_STATUS_LOCK, asyncio.Lock)
...
def test_second_invocation_skips_while_first_is_running(fresh_db):
    ...
    async def _run():
        ...
        with patch.object(
            import_discovery, "_check_download_status_impl", new=_slow_impl
        ):
...
def test_sequential_invocations_still_run(fresh_db):
    ...
    async def _run():
        ...
        with patch.object(
            import_discovery, "_check_download_status_impl", new=_fast_impl
        ):
...
def test_exception_in_impl_releases_the_lock(fresh_db):
    ...
    async def _run():
        ...
        with patch.object(
            import_discovery, "_check_download_status_impl", new=_flaky_impl
        ):
...
def test_create_task_amplification_is_bounded(fresh_db):
    ...
    async def _run():
        ...
        with patch.object(
            import_discovery, "_check_download_status_impl", new=_very_slow_impl
        ):
            task1 = asyncio.create_task(main.check_download_status())
            await impl_calls["inside"].wait()
            tasks = [
                asyncio.create_task(main.check_download_status()) for _ in range(3)
            ]
...
def test_lock_is_a_single_module_level_object():
    ...
    lock_id_first = id(import_discovery._CHECK_DOWNLOAD_STATUS_LOCK)
    lock_id_second = id(import_discovery._CHECK_DOWNLOAD_STATUS_LOCK)
